...
class FPS:
    ...
    def _get_surface_name(self, package_name: str) -> str:
        ...
        if last:
            if package_name != last.group('package'):
                self.error = (f'Surface not found for package {package_name}. '
    ...
    def _get_recent_frames(self):
        """Returns a list of Frames."""
        if not self._surface_name:
            self.error = ('Please make sure the game is running!!')
            return
    ...
    def print_stats(self, frames: List[Frame]) -> None:
        ...
        if self._print_latency:
            ...
            latency_str = ('/ LATENCY (ms) '
                           ...
                           f'time elasped={time.time() - self._start_time:.2f}s')
