...
class Memory:
    ...
    def _update_mem_calculations(self) -> None:
        ...
        """
        assert self._app_values, "Failed to get values for app memory usage."
        assert self._device_values, ("Failed to get values for device"
                                     " memory usage.")
        assert self._total_mem, "Failed to get total memory available."
        assert len(self._app_values) == len(self._device_values), \
            ("Invalid data, expected matching length of app usage and device"
             " usage values.")
